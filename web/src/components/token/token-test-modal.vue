...
const hasResult = computed(() => props.metaText.trim().length > 0 || props.resultText.trim().length > 0)
  ...
  <UiModal :open="open" title="测试 Token" size="lg" @close="$emit('close')">
    <div class="space-y-4">
      <div>
        ...
        <input type="text" class="geist-input font-mono bg-gray-50 text-gray-500" :value="tokenDisplay" readonly>
      </div>
      <div>
        <label class="modal-label mb-1 block">测试模型（仅聊天模型）</label>
        <select class="geist-input" :value="selectedModel" :disabled="running || models.length === 0" @change="onModelChange">
